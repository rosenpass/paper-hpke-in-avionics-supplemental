...
pub fn benchmark_kem<Kem: hpke::Kem>(c: &mut Criterion, info: &[u8], ad: &[u8], pt: &[u8]) {
    ...
    let black_box_seal =
        || -> Vec<u8> { black_box(&endpoint).seal(bb(ad), bb(&pk), bb(pt)).unwrap() };
    let black_box_open = |ct: Vec<u8>| bb(&endpoint).open(bb(ad), bb(&pk), bb(&ct)).unwrap();
