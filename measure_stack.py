...
def run_program(program_path, size, find_all_segments=False):
    ...
    try:
        subprocess.run(
            ["softlimit", softlimit_option, str(size), program_path],
            check=True,
...
def find_max_size(program_path, successful_runs, start_size, find_all_segments=False):
    ...
    with tqdm(
        total=log2(lower_bound + upper_bound), desc="Binary Search", leave=False
    ) as search:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Find the maximum stack/all_segments size for a given program."
    )
    parser.add_argument(
        "--runs",
        "-r",
        type=int,
        default=100,
        help="Number of successful runs to validate functionality",
    )
    parser.add_argument(
        "--limit",
        "-l",
        type=int,
        default=8192 * 1028,
        help="Starting size limit in bytes",
    )
    parser.add_argument(
        "--all_segments",
        "-as",
        action="store_true",
        help="Find memory consumption of all segments instead of just the size",
    )
    parser.add_argument("program_path", help="Path to the program to run")

    args = parser.parse_args()

    max_value = find_max_size(
        args.program_path, args.runs, args.limit, args.all_segments
    )
