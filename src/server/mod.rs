...
impl<E: Endpoint> CipherServer<E> {
    ...
    fn process_internal(&self, endpoint: u32, opcode: OpCode, payload: &[u8]) -> Result<Vec<u8>> {
        ...
        match opcode {
            OpCode::Encrypt => Self::process_seal(endpoint, payload),
            OpCode::Decrypt => Self::process_open(endpoint, payload),
    ...
    fn process_seal(endpoint: &E, request: &[u8]) -> Result<Vec<u8>> {
        let (additional_data, rest) = request.extract_sized_field()?;
        let (sealion_key, message) =
            Self::extract_peer_public_key(endpoint.public_key_size(), rest)?;
        endpoint.seal(additional_data, sealion_key, message)
    }

    fn process_open(endpoint: &E, request: &[u8]) -> Result<Vec<u8>> {
        let (additional_data, rest) = request.extract_sized_field()?;
        let (public_key, sealed_message) =
            Self::extract_peer_public_key(endpoint.public_key_size(), rest)?;
        let message = endpoint.open(additional_data, public_key, sealed_message)?;
        Ok(message)
    }

    fn process_peer_public_key_request(&self, request: &[u8]) -> Result<Vec<u8>> {
        debug!("answer public sealion key request");
...
mod tests {
    ...
    #[test]
    fn test_cipher_server_process_seal() {
        ...
        endpoint
            ...
            .returning(|_, _, _| Ok(Vec::from("sealed-data")));
        ...
        assert_eq!(&response[2..], b"sealed-data");
    }

    #[test]
    fn test_cipher_server_process_open() {
        let mut endpoint = MockEndpoint::new();
        ...
        endpoint
            .expect_open()
            .withf(|additional_data, public_key, sealed_message| {
                additional_data == [10, 11]
                    && public_key == [4, 5, 6]
                    && sealed_message == [7, 8, 9]
            })
            .returning(|_, _, _| Ok(Vec::from("opened-data")));
    ...
    fn test_cipher_server_process_request_peer_public_key() {
        ...
        endpoint2
            ...
            .return_const(Vec::from("public-sealion-key-2"));
        ...
        // Perform the test to check public sealion keys of both endpoints
        ...
        assert_eq!(&response[2..], b"public-sealion-key-2");
    }
    ...
    #[test]
    fn test_cipher_server_process_seal_empty_request_success() {
        let mut endpoint = MockEndpoint::new();
    ...
    #[test]
    fn test_cipher_server_process_seal_empty_request_fail() {
        let mut endpoint = MockEndpoint::new();
    ...
    #[test]
    fn test_cipher_server_process_open_empty_request() {
        let mut endpoint = MockEndpoint::new();
    ...
    #[test]
    fn test_cipher_server_process_open_empty_sealed_message_fail() {
        let mut endpoint = MockEndpoint::new();
        ...
        endpoint
            ...
            .withf(|additional_data, public_key, sealed_message| {
                additional_data == [10, 11, 12]
                    && public_key == [1, 2, 3, 4]
                    && sealed_message.is_empty()
            })
            .returning(|_, _, _| Err(anyhow::anyhow!("Empty sealed message")));

        // Insert the mock endpoint into the CipherServer
        let mut cipher_server = CipherServer::new();
        cipher_server.insert_endpoint(1, endpoint);

        // Perform the test for OpCode::Decrypt with an empty sealed message.
...
mod integration_tests {
    ...
    fn test_cipher_server_seal() {
        ...
        endpoint
            ...
            .returning(|_, _, _| Ok(Vec::from("sealed-data-1")));
    ...
    fn test_cipher_server_open() {
        ...
        let sealed_message = [5, 6, 7];
        let mut request_builder_buffer = [0u8; 32768];
        let mut request_builder = RequestBuilder::new(&mut request_builder_buffer);
        let request = request_builder
            .build_open_request(&public_key, &sealed_message, &additional_data)
            .unwrap();
        let response = cipher_server.process_endpoint_request(1, request);
        let response = Response::try_from(response.as_slice()).unwrap();
        if let Response::DecryptedMessage(message) = response {
            assert_eq!(message, b"opened-data");
        } else {
    ...
    #[test]
    fn test_cipher_server_request_peer_sealion_key() {
